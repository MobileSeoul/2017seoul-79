...
public class profile_a_adapter extends RecyclerView.Adapter<profile_a_adapter.ViewHolder> {

    private Context context;
    private List<pro_a> item;
    private Integer[] tagImage = new Integer[12];

    public profile_a_adapter(List<pro_a> item, Context context) {
        super();
        this.item = item;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.profile_a_item, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        final pro_a item1 = item.get(position);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageView imageView;
        @Nullable
        @BindView(R.id.a_imageCount)
        TextView imageCount;
        @BindView(R.id.a_like_img)
        ImageView likeimg;
        @BindView(R.id.a_like_text)
        TextSwitcher liketext;
        @BindView(R.id.a_comment_text)
        TextView commenttext;
        @BindView(R.id.a_date_text)
        TextView datetext;
        @BindView(R.id.a_like_lin)
        LinearLayout likeClick;
        @BindView(R.id.a_tag)
        ImageView tag;
        @BindView(R.id.a_cardview)
        CardView view;
